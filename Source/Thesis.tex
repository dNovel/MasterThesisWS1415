...
Entwickelt werden Anwendungen aus einer Mischung von Blueprint Items und C++ Code. 
Blueprint ist node basiert, deswegen nicht ganz drauf eingehen
C++ wird in VS entwickelt. Der Code kann kompiliert werden im Editor und dann gleich eingesetzt werden. C++ hat Schnittstellen zu Blueprint. Es bietet die Möglichkeit im Editor editierbare Variablen und Funktionen anzulegen.
Neues Projekt und bei Erstellung wird gefragt wie man beginnen möchte. C++ oder Blueprint. Hier zeichnet sich schon eine Trennung ab. Eventgraphen noch betrachten oder kurz erwähnen.

\subsubsection{Asset Verwaltung in der Unreal Engine 4}
Assets können in der Unreal Engine 4  z.B. im Dateiformat .fbx (Version 2013) Importiert werden. Die Modelle und Materialien können hierbei z.B. direkt in 3DS Max, Modo oder Cinema 4D erstellt und übernommen werden. Es ist also möglich, die UE4 direkt in die AssetPipeline einzubinden. Durch die offene Verfügbarkeit des Quellcodes können auch eigene Module für die UE4 geschrieben werden, welche die Assetpipleine erweitern. Die Unreal Engine bietet einen Contentbrowser (siehe Abbildung \ref{UEContentbrowser}) an, welcher es ermöglicht Assets zu sortieren und diese zu durchsuchen. Die Assets können dann per Drag and Drop in die Szene der Engine eingefügt werden.
...
Zur Versionsverwaltung der Assets ist die Möglichkeit zur Integration eines Versionskontrollsystems gegeben. Eingebunden werden können auf GIT, Subversion und Perforce basierte Systeme.
...
Dieser Abschnitt beschreibt das Vorgehen während des Systemdesigns. Das Ziel hier ist es, eine technische Konzeption für ein Softwareprodukt zu erläutern. Dieses technische Konzept ist Grundlage für eine folgende Implementierung des Softwareprojektes.
