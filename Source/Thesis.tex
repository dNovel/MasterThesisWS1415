...
%%%%%%
\chapter{Einführung in Game Authoring und dessen Arbeitsprozesse}
Arbeitsprozesse in heutigen Game Engines verlangen von Entwicklern häufig das Erlernen neuer Tools und Toolsets und dies während eines zeitlich knapp bemessenen Projektzeitraums. Es wäre für Entwickler möglicherweise einfacher sich nur mit bereits aus dem Arbeitsalltag bekannten Tools zu beschäftigen. Alltägliche Tools definieren sich je nach Entwicklergruppe unterschiedlich. Für Artists und Designer handelt es sich hierbei meist um Welt-Editoren sowie Grafik und 3D-Modeling Applikationen. Programmierer in der Spiele-Entwicklung arbeiten meist mit Entwicklungsumgebungen, Debuggern und Analyse Tools.\\

Spezielle Game Authoring Tools\footnote{Werkzeuge zum Erstellen von Interaktiven Softwareapplikationen} ermöglichen aber auch Teammitgliedern ohne tiefgehendes technisches Verständnis für die Programmierung von Spielen, an Projekten in der Entwicklung mitzuarbeiten. So können Designer und Artists Skripte und Verhaltensalgorithmen für die Spiellogik entwerfen oder die Performance der Applikation testen. Authoring Tools können das Entwickeln von Anwendungen erheblich beschleunigen und Entwickler bei alltäglichen Aufgaben unterstützen. Vgl. \fullcite{FutureTrendsGAT:2012}
...
\end{quote}
Diese Entwicklung birgt einen interessanten Gedanken und zeigt, dass Tool Development in der Spiele-Industrie in den letzten Jahren durch immer komplexere Projekte und Features einen wichtigen Stellenwert im Projekt erreicht hat.
...
\end{quote}
Seine Theorie spricht also für die Integration von Tool Funktionalität in die meist genutzte Software der Entwickler. Es würde einiges an Lernaufwand für die Game Developer und Entwicklungsaufwand für die Tool Developer sparen, könnten Standard Werkzeuge der Industrie wie z.B. der Modeling Editor Cinema 4D um Tool Funktionalität erweitert werden.

\section{Fragestellung}
Diese Arbeit beschäftigt sich mit der Analyse und Entwicklung eines Arbeitsprozesses, seiner dazugehörigen Methoden und Schritte und abschließend mit der Konzeption und Implementierung eines Authoring Tools für die Fusee Engine\footnote{Fusee (Furtwangen Simulationd and Entertainment Engine - \url{http://www.Fusee3d.org})}. Hierbei soll untersucht werden, wie es möglich ist ein solches Authoring Tool zu konzipieren wenn es in einen bereits bestehenden proprietären Modeling Editor (hier Cinema 4D von Maxon \footfullcite{MaxonC4d2014}) mit Hilfe dessen API integriert werden soll. Hierfür gelten einige Voraussetzungen. Das konzipierte Tool und die Anbindung an Cinema 4D müssen in der Programmiersprache \CSS implementiert werden. Das Modul Uniplug, ein Teilprojekt der Fusee Engine, muss verwendet und das Authoring Tool trotzdem nicht an den Cinema 4D Editor gebunden sein. Des Weiteren soll das Tool durch andere Entwickler leicht auf andere Editoren anpassbar sein. Somit wird auf eine enge Bindung zwischen dem Framework und dem Modeling Editor Cinema 4D verzichtet.
...
\section{Während dieser Arbeit verwendete Software}
\begin{itemize}
...
Tools können oft für eine spätere Aufgabe wiederverwendet werden. So genannte “Throw-away tools” (z. Dt. Wegwerf-Tools) wurden meist speziell für eine Aufgabe angefertigt ohne dabei die Wiederverwendbarkeit im Auge zu behalten. Vgl. \autocite[S. 3]{Wihlidal2006}
...
\end{quote}
Somit ist die Assetpipeline ein wichtiges Instrument und besteht meist aus einer Zusammenstellung von eigens Entwickelten Tools und lizensierter Software wie z.B. Game Engines, Modeling Editoren wie 3DS Max, Cinema 4D, oder Maya und weiterer Software. Diese Arbeit beschäftigt sich also mit einem Teilgebiet der Asset Pipeline. In einem späteren Kapitel wird das Fusee Asset Management betrachtet und die Asset Pipeline für das Entwickeln einer Fusee Anwendung mit Cinema 4D konzeptionell untersucht.
...
Grundsätzlich ist eine solche Analyse auch für das eigentlich Team sinnvoll. Während der Konzeption eines Tools muss darauf geachtet werden, welche Gruppen von Entwicklern mit welchem Grad an technischem und gestalterischem Verständnis mit dem Tool umgehen sollen. Daher ist es essentiell über die Strukturen des eigenen Entwickler Teams in Kenntnis zu sein.
...
Um eine Produktion erfolgreich zu starten, bedarf es einer Asset Pipeline und der dazugehörigen Tools. Um zu untersuchen, welche Tools für das Projekt nötig sind, sollten sich Lead-Developer zu Beginn des Projekts folgende von \cite[S 223-224]{Chandler2006} beschriebene Fragen stellen.
\begin{itemize}
...
\end{quote}
Hiermit zeigt Chico ein Kernthema dieser Arbeit auf: Wichtig sind Tools, welche auf dem Team bereits bekannter Software basieren, in ihrer Bedienung leicht zu verstehen und jedem Mitglied des Teams zur Verfügung stehen. Solche Authoring Tools könnten den Produktionsprozess zugänglicher gestalten. Da eine Erweiterung eines intern entwickelten Tools durch die Entwickler dauerhaft möglich wäre, kann auch ein langfristiger Einsatz derer gewährleistet werden und rechtfertigt somit eine aufwändigere Entwicklungsarbeit. Die Eckpfeiler dieser Tools wiederum ergeben sich aus der Assetpipeline der Entwickler, Designer und Artists. Somit ergibt sich eine enge Beziehung zwischen Third Party Software und den eigenen Tools. Wie Chico bereits erwähnt, ist es hierbei essentiell die Tools alle sinnvoll zu verknüpfen um die Wege zwischen der Game Engine und den Asset Creation Applikationen um die Qualität und Moral der beteiligten Mitarbeiter zu erhalten. Somit wäre es sehr praktisch die Editoren der Game Engine direkt in die Content Creation Software wie Cinema 4D zu integrieren. Ein kürzerer Weg kann dann nicht mehr erreicht werden.

\subsection{Der Analyse und Entwicklungsprozess des Tool Developments}
Nach Wihlidal \parencite[S. 37-40]{Wihlidal2006} unterscheidet sich die Planung eines Projektes zur Erstellung eines Developer Tools nicht sehr von der Planung für die allgemeine Entwicklung von Software. Es gelten hier vier Planungsphasen die den Projektablauf kennzeichnen. Der erste Schritt wäre eine allgemeine Planung des Tools. Diese beinhaltet Funktionen und Umfang des Tools. Eine Beschreibung der Anforderungen bzw. der Ziele des Projektes wird mit den begünstigten Entwicklern abgesprochen.
...
%TODO oder mit Artist zusammen führen.

\subsection{Use-Cases eines Engineer}
Diese Auflistung beschreibt die Tasks des Engineer.
\begin{itemize}
...
%TODO: Hierzu werden die beiden Systeme analysiert und dann ein Design zum Mapping des Cinema 4D Graphen auf das Fusee System erläutert.
